...
public class B2Client : IB2Client {
	...
	public static async Task<B2Options> AuthorizeAsync(B2Options options) {
		...
		HttpRequestMessage requestMessage = AuthRequestGenerator.Authorize(options);
		HttpResponseMessage response = await client.SendAsync(requestMessage);

		string jsonResponse = await response.Content.ReadAsStringAsync();
		if (response.IsSuccessStatusCode) {
			B2AuthResponse authResponse = JsonConvert.DeserializeObject<B2AuthResponse>(jsonResponse);
